...
def get_data_dicts(img_path):
    ...
    dataset_dicts = []
    print("print from body interface line 40",img_path)
...
def run_model(cfg, relation_h, **kwargs):
    ...
    data_folder = os.path.join(kwargs['dataset'], 'img').replace('\\', '/')
    ...
    # predictions = inference_on_dataset(model, data_loader)
            ...
            for file_name in file_list:
                ...
                with open('{:s}_elements.json'.format(os.path.join(data_folder, image_name)), 'w') as file:
                        json.dump(json_dicts, file)
